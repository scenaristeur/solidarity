var Solidarity={},sol=Solidarity;function clone(a){return $.extend({},a)}function trace(a){console.log(a)}function setScrollerToBottom(a){a=document.getElementById(a);a.scrollTop=a.scrollHeight}function padLeadingZero(a){return 10>a?"0"+a:a}
var g={w:-1,h:-1,device:{isMobile:!1,isTablet:!1,isPC:!1},preferences:{date_format:1},state:{bFirstRunSession:!1,pc_tab_bar_main:1,pc_tab_bar_main_condensed:1,pc_tab_bar_main_right:1},solid:{sh:{},pod:{},session:{},pti:{},name:"",photo:"",friends:{},role:{},storage:{},inbox:{},isLogged:!1,webId:"",instances:{longchat:"",chat:""}},date:{day:"",month:"",year:""},active_channel:{sh:{},pod:{},pti:{},name:"",photo:"",friends:{},role:{},storage:{},long_chat_instance:{},chat:{},chat_pages:{},chat_massages:{},
long_chat_instance_isPrivate:!1},favourite_channels:{selected:0,list:[{channel_name:"Solidarity",channel_url:"https://solidarity.inrupt.net"},{channel_name:"Shighl/Shighl",channel_url:"https://solidarity.inrupt.net"}]}},UIX={Mobile:{BurgerMenuShow:function(){TweenMax.to($("#mobile_burger_menu_slideout"),.25,{width:g.w,delay:.5})},UserMenuShow:function(){},MainMenuShow:function(){}}};function string__getURLPath(a){return a.match(/(https?:\/\/[^\s]+)/g)}
...
Solidarity={Responsive:{ArrangeViewport:function(){g.device.isPC&&($("#uix_baseframe_banner_tr").css("left",g.w-49),$("#uix_baseframe_banner_bm").css("width",g.w-51),$("#uix_baseframe_banner_rh").css("left",g.w-26),$("#uix_baseframe_banner_lh").css("height",g.h-49),$("#uix_baseframe_banner_rh").css("height",g.h-49),$("#uix_baseframe_frame_bl").css("top",g.h-47),$("#uix_baseframe_frame_br").css("top",g.h-47).css("left",g.w-48),$("#uix_baseframe_frame_bm").css("top",g.h-25),$("#div_chatviewportbg").css("height",
g.h-190).css("width",g.w-258),$("#div_chatviewport_inputconsole").css("top",g.h-120).css("width",g.w-315),$("#div_main_tab_burger").css("left",g.w-80),$("#div_chatter_container").css("width",g.w-267).css("height",g.h-280));g.device.isMobile&&($("#mobile_burger_menu_slideout").css("width",0).css("height",g.h),$("#mobile_burger_menu_slideout_bg").css("width",g.w).css("height",g.h))},DeviceView:function(){0<g.w&737>g.w&&(g.device.isMobile=!0,$("#div_pc").css("display","none"),$("#div_tab").css("display",
"none"),$("#div_mob").css("display","inline"));736<g.w&1068>g.w&&(g.device.isTablet=!0,$("#div_pc").css("display","none"),$("#div_tab").css("display","inline"),$("#div_mob").css("display","none"));1067<g.w&&(g.device.isPC=!0,$("#div_pc").css("display","inline"),$("#div_tab").css("display","none"),$("#div_mob").css("display","none"))}}};window.onresize=function(){g.w=$(window).width();g.h=$(window).height();Solidarity.Responsive.DeviceView();Solidarity.Responsive.ArrangeViewport()};
...
function init(){function a(a){console.log("WebId: ",a);null!=a?(console.log("Already Logged in as "+a),g.solid.isLogged=!0,g.solid.webId=a,g.solid.pod.webId=a,solid_login__getNameAndPhoto()):g.solid.session.login()}$(".tabular.menu .item").tab({history:!1});$.tab("change tab","tab_chatter");g.solid.sh=new Shighl;g.solid.session=new g.solid.sh.session;g.solid.pod=new g.solid.sh.pod;(async function(){await g.solid.session.track(a)})();$("#div_chatter_container").scroll(function(){0==$("#div_chatter_container").scrollTop()&&
trace("top fo scroller")});solid_channel__attachAddNewChannelTextfieldEvents();var b=new Date;g.date.day=padLeadingZero(b.getDate())+"";g.date.month=padLeadingZero(b.getMonth()+1)+"";g.date.year=b.getFullYear()+"";trace("Year-Month-Day: "+g.date.year+"-"+g.date.month+"-"+g.date.day);window.sfx_new_chat=new Howl({src:["audio/Pling-KevanGC-1485374730.mp3"]});cookie.get("fBoot")?g.favourite_channels=JSON.parse(cookie.get("Channels")):(cookie.set("fBoot",!0),cookie.set("Channels",JSON.stringify(g.favourite_channels)));
solid_channels__displayFavouriteChannels();solid_long_chat__setChannel(g.favourite_channels.list[g.favourite_channels.selected].channel_url,g.favourite_channels.list[g.favourite_channels.selected].channel_name)}function init__splash_enterClicked(){$("#div__index_splash_pc").css("display","none")}function div_todolist_add(){alert("add clicked!")}
function div_todolist__onClick(a){for(var b=0;b<g.m_todolist._data._items.length;b++)$("#"+g.m_todolist._data._parent+"__item"+b).prop("checked",!1);$("#"+g.m_todolist._data._parent+"__item"+a).prop("checked",!0);g.m_todolist._data._selected=a}function navigation__chatterModeClicked(){}function navigation_tabs_onclick(a){}function navigation__helpPages(){}
function solid_login__ShowAsLoggedIn(a,b){$("#div_banner__account__loggedin__name").html(b);null!=a?$("#div_banner__account__loggedin__avatar").css("background-image","url('"+a+"')"):$("#div_banner__account__loggedin__avatar").css("background-image","url('images\no-avatar.png')");$("#div_banner__account__loggedout").css("display","none");$("#div_banner__account__loggedin").css("display","inline")}
function solid_login__ShowAsLoggedOut(){$("#div_banner__account__loggedin__name").html("");$("#div_banner__account__loggedin__avatar").css("background-image","url('')");$("#div_banner__account__loggedin").css("display","none");$("#div_banner__account__loggedout").css("display","inline")}
async function solid_login__getNameAndPhoto(){trace("solid_login__getNameAndPhoto() -> FIRED!");g.solid.photo=await g.solid.pod.photo;g.solid.name=await g.solid.pod.name;solid_login__ShowAsLoggedIn(g.solid.photo,g.solid.name)}async function solid_login__login(){console.log("trying to login..");g.solid.session.login()}async function solid_login__logout(){trace("solid_login__logout() -> FIRED!");g.solid.session.logout();g.solid.isLogged=!1;solid_login__ShowAsLoggedOut()}
...
async function solid_long_chat__initLongChatInstance(a){a||(g.active_channel.chat=new g.active_channel.sh.chat,g.active_channel.chat.instance=g.active_channel.long_chat_instance,g.active_channel.chat_pages=await g.active_channel.chat.init,g.active_channel.chat.subscribe=solid_long_chat__subscribe_onReceivedMessage,trace("chat.init -> Called!"),trace(g.active_channel.chat_pages));g.active_channel.chat_massages=await g.active_channel.chat.messages;trace(g.active_channel.chat_massages);a="";$("#div_chatter_container").html("");
for(var b=0;b<g.active_channel.chat_massages.length;b++){var c=g.active_channel.chat_massages[b];trace(c);if(void 0!=c.maker){var d=Templates.Get("Chatter_Item");"undefined"!=c.makerimg?d=d.searchAndReplace("@AVATAR@",c.makerimg):d=d.searchAndReplace("@AVATAR@","images/no-avatar.png");d=d.searchAndReplace("@AVATAR@",c.makerimg);d=d.searchAndReplace("@LBL_NAME@",c.makername);d=d.searchAndReplace("@LBL_CONTENT@",c.content);d=d.searchAndReplace("@LBL_TIME@",0==g.preferences.date_format?jQuery.timeago(new Date(c.date)):
moment(new Date(c.date)).add(24,"hours").format("MMM Do YYYY, h:mm a"));a+=d}}$("#div_chatter_container").html("<div style='margin-top:0px'></div>"+a+"<div style='margin-top:25px'></div>");0==g.state.bFirstRunSession?g.state.bFirstRunSession=!0:window.sfx_new_chat.play();setScrollerToBottom("div_chatter_container")}function solid_long_chat_reply__onEnterCheck(){return 13===window.event.keyCode?(trace("ENTER Pressed!"),solid_long_chat__sendMessage(),!1):!0}
async function solid_long_chat__sendMessage(){trace("solid_long_chat__sendMessage() -> FIRED!");if(g.solid.isLogged){var a=$("#frm_lc_text_input").val().trim();$("#frm_lc_text_input").val("");$("#frm_lc_text_input").blur();var b=g.active_channel.chat_massages[g.active_channel.chat_massages.length-1],c=b.url;b=b.maker;g.active_channel.chat.message={content:a,webId:g.solid.webId,postType:"InstantMessage",replyTo:c};if(0<b.length){console.log("author",b);trace("--------------------------------");trace("replyTo="+
c);trace(" author="+b);trace("postType=InstantMessage");trace("--------------------------------");a=g.active_channel.chat.lastPost;let d=new sh.pod;console.log("pod",d);d.webId=b;console.log("pod",d);b=new sh.inbox;b.inbox=await d.inbox;console.log("ib",b);b.message={sender:g.solid.webId,title:"solidarity reply",url:a,content:a+" is a InstantMessage reply to' "+c,replyTo:c}}}else alert("you must be logged into your Solid account to send messages!!")}
function solid_long_chat__subscribe_onReceivedMessage(){solid_long_chat__initLongChatInstance(!0)}function solid_long_chat__setChannel(a,b){trace("!!!!! ONLY BE CALLED ONCE!!!!!!  solid_long_chat__setChannel() -> FIRED!");solid_long_chat__getPublicTypeIndex(a+"/profile/card#me",a,b)}Solidarity.Friends={Get:function(a){(async function(){return await sh.getFriends(a)})()}};
