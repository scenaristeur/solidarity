var gblVersion="0.3",Solidarity={},sol=Solidarity,sh={};function clone(a){return $.extend({},a)}function trace(a){console.log(a)}function setScrollerToBottom(a){a=document.getElementById(a);a.scrollTop=a.scrollHeight}function padLeadingZero(a){return 10>a?"0"+a:a}function openwindow(a){return(a=window.open(a,"_blank"))?(a.focus(),!0):!1}
...
g.w-80),$("#div_chatviewportbg").css("height",g.h-190).css("width",g.w-258),$("#div_chatter_container").css("width",g.w-267).css("height",g.h-280),$("#div_inputconsole__loggedin__avatar_container").css("left",225).css("top",g.h-110),$("#div_chatviewport_inputconsole").css("top",g.h-120).css("width",g.w-343));g.device.isMobile&&($("#mobile_burger_menu_slideout").css("width",0).css("height",g.h),$("#mobile_burger_menu_slideout_bg").css("width",g.w).css("height",g.h))},DeviceView:function(){0<g.w&737>
g.w&&(g.device.isMobile=!0,$("#div_pc").css("display","none"),$("#div_tab").css("display","none"),$("#div_mob").css("display","inline"));736<g.w&1068>g.w&&(g.device.isTablet=!0,$("#div_pc").css("display","none"),$("#div_tab").css("display","inline"),$("#div_mob").css("display","none"));1067<g.w&&(g.device.isPC=!0,$("#div_pc").css("display","inline"),$("#div_tab").css("display","none"),$("#div_mob").css("display","none"))}}};
window.onresize=function(){g.w=$(window).width();g.h=$(window).height();Solidarity.Responsive.DeviceView();Solidarity.Responsive.ArrangeViewport()};Templates.template_list.push({name:"ListSimple",value:'<p>@lbl_header@</p><ul id="@ID@" style="list-style-type:none;">@BODY@</ul><p>@lbl_footer@</p><div><button onclick="javascript:@INSTANCE@_add();" type="button">Add</button><button onclick="javascript:@INSTANCE@_edit();" type="button">Edit</button><button onclick="javascript:@INSTANCE@_remove();" type="button">remove</button></div>'});
...
function init(){function a(a){console.log("WebId: ",a);null!=a?(console.log("Already Logged in as "+a),g.solid.isLogged=!0,g.solid.webId=a,g.solid.pod.webId=a,solid_login__getNameAndPhoto()):g.solid.session.login()}$(".tabular.menu .item").tab({history:!1});$.tab("change tab","tab_chatter");g.solid.sh=new Shighl;g.solid.session=new g.solid.sh.session;g.solid.pod=new g.solid.sh.pod;var b=g.solid.sh.version;$("#sp_lblSplashVersion").html("v&nbsp;"+gblVersion+" / "+(null==b?"n/a":b));(async function(){await g.solid.session.track(a)})();
$("#div_chatter_container").scroll(function(){0==$("#div_chatter_container").scrollTop()&&trace("top fo scroller")});solid_channel__attachAddNewChannelTextfieldEvents();b=new Date;g.date.day=padLeadingZero(b.getDate())+"";g.date.month=padLeadingZero(b.getMonth()+1)+"";g.date.year=b.getFullYear()+"";trace("Year-Month-Day: "+g.date.year+"-"+g.date.month+"-"+g.date.day);window.sfx_new_chat=new Howl({src:["audio/Pling-KevanGC-1485374730.mp3"]});cookie.get("fBoot")?g.favourite_channels=JSON.parse(cookie.get("Channels")):
(cookie.set("fBoot",!0),cookie.set("Channels",JSON.stringify(g.favourite_channels)));solid_channels__displayFavouriteChannels();solid_long_chat__setChannel(g.favourite_channels.list[g.favourite_channels.selected].channel_url,g.favourite_channels.list[g.favourite_channels.selected].channel_name)}function init__splash_enterClicked(){$("#div__index_splash_pc").css("display","none")}function div_todolist_add(){alert("add clicked!")}
...
function solid_login__ShowAsLoggedIn(a,b){$("#div_banner__account__loggedin__name").html(b);null!=a?($("#div_banner__account__loggedin__avatar").css("background-image","url('"+a+"')"),$("#div_inputconsole__loggedin__avatar").css("background-image","url('"+a+"')")):($("#div_banner__account__loggedin__avatar").css("background-image","url('images\no-avatar.png')"),$("#div_inputconsole__loggedin__avatar").css("background-image","url('images\no-avatar.png')"));$("#div_banner__account__loggedout").css("display",
"none");$("#div_banner__account__loggedin").css("display","inline")}function solid_login__ShowAsLoggedOut(){$("#div_banner__account__loggedin__name").html("");$("#div_banner__account__loggedin__avatar").css("background-image","url('')");$("#div_banner__account__loggedin").css("display","none");$("#div_banner__account__loggedout").css("display","inline")}
...
function modals__manage_channels_show(){$("#modal_manage_channels").modal({closable:!0,onDeny:function(){},onApprove:function(){}}).modal("show")}function navigation__chatterModeClicked(){}function navigation_tabs_onclick(a){}function navigation__findOutMore(){openwindow("https://github.com/scenaristeur/solidarity")};